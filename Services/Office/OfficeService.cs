...
namespace BaseDotnet.Services
{
    public class OfficeService : IOfficeService
    {
        ...
        public OfficeService(BaseDotnetDbContext context, IOptions<AppSettings> appSettings)
        {
            _context = context;
            _appSettings = appSettings.Value;
        }

        public async Task<int> Add(Office item)
        {
            var officeData = new Office
            {
                OfficeName = item.OfficeName,
                OfficeAddress = item.OfficeAddress,
                OfficeLongitude = item.OfficeLongitude,
                OfficeLatitude = item.OfficeLatitude,
                CreatedAt = DateTime.Now,
                UpdateAt = DateTime.Now,
            };

            _context.Office.Add(officeData);
            _context.SaveChanges();
            return officeData.OfficeId;
        }
